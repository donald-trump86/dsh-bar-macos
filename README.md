...
- 🚪 **退出语义明确**：`⌘Q` 退出菜单栏图标、**保留后台服务**（下次打开自动重新接管）；需要连服务一起关掉时用 `⌘⌥Q` **Quit & Stop Service…**。外部启动的服务在任何退出路径下都不会被碰。
...
- 🌏 **简体中文界面**：偏好设置里可切换 **自动 / English / 简体中文**，切换后**立即生效、无需重启**（面板、菜单栏、日志窗口同时跟随）。
  - 为什么需要手动切换：本机 `AppleLanguages` 是 `en-CN` 优先、`zh-Hans-CN` 次之，而 macOS 取"第一个有资源的语言"——所以"自动"会解析成英文。想要中文必须显式选择。
  - 文案集中在 `Sources/Localization.swift` 一张类型安全的表里（枚举做键，拼错即编译错误）；缺失翻译会回退到英文，而不是显示键名。
