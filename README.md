...
- **键盘优先**：支持丰富菜单快捷键、控制面板，以及随时随地全局直达热键。
...
- 🪟 **原生毛玻璃控制面板（⌘D）**：
  ...
  - **启动时自动打开网页**（Auto open Web on launch）：可按习惯自由开启或关闭。
...
| 快捷键 | 动作 | 说明 |
| :--- | :--- | :--- |
| **`⌥ + ⇧ + D`**<br>*(支持在面板中自定义)* | **一键直达 DSH Web** | 若服务未启动则后台拉起并打开浏览器；若已在运行则直接前置浏览器 |
| **`⌥ + ⇧ + H`** | **唤出控制面板** | 随时弹出原生悬浮设置面板 |
...
│   ├── ServiceManager.swift    # Harness 服务健康检测、生命周期管理与日志
│   ├── HotKeyManager.swift     # Carbon 原生无权限门槛全局热键管理
│   ├── SettingsManager.swift   # 开机启动 (SMAppService)、快捷键与偏好设置持久化
│   └── DashboardWindow.swift   # 原生毛玻璃悬浮控制中心与快捷键录制面板
├── Resources/
